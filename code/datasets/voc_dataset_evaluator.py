...
from datasets.dataset_catalog import DEVKIT_DIR, IM_DIR
...
def _write_voc_results_files(json_dataset, all_boxes, salt):
    ...
    with open(image_set_path, 'r') as f:
        image_index = [[x, int(x.strip())] for x in f.readlines()]
    image_index.sort(key=lambda x: x[1])                        # Sometimes image index is not sorted
    image_index = [one_index[0] for one_index in image_index]   # It needs to be sorted by number
    ...
    for i, entry in enumerate(roidb):
        ...
        assert int(index) == int(image_index[i])
...
def _do_python_eval(json_dataset, salt, output_dir='output'):
    ...
    # But here we still use 2007 metric, so skip this check
...
def voc_info(json_dataset):
    ...
    #     devkit_path, 'VOC' + year, 'Annotations', '{0:s}.xml')
    # image_set_path = os.path.join(
    #     devkit_path, 'VOC' + year, 'ImageSets', 'Main', image_set + '.txt')
    year = ''
    image_set = 'test' if 'test' in json_dataset.name else 'train'
    devkit_path = DATASETS[json_dataset.name][DEVKIT_DIR]
    anno_path = os.path.join('/'.join(DATASETS[json_dataset.name][IM_DIR].split('/')[:-1]), 'Annotations', '{0:s}.xml')
    image_set_path = os.path.join('/'.join(DATASETS[json_dataset.name][IM_DIR].split('/')[:-1]), 'ImageSets', 'Main', image_set + '.txt')

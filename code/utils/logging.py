...
def log_stats(stats, misc_args):
    ...
    lines += "\t\tloss: %.6f, lr: %.6f time: %.6f, eta: %s, delta: %.6f, lambda: %.6f,\n" % (
        stats['loss'], stats['lr'], stats['time'], stats['eta'], stats['delta'], stats['lambda']
    )
    if stats['head_losses']:
        lines += "\t\t" + ", ".join("%s: %.6f" % (k, v) for k, v in stats['head_losses'].items()) + "\n"
    if stats['head_DA_losses']:
        lines += "\t\t" + ", ".join("%s: %.6f" % (k, v) for k, v in stats['head_DA_losses'].items()) + "\n"

...
class TrainingStats(object):
    ...
    def __init__(self, misc_args, log_period=20, tensorboard_logger=None):
        ...
        self.smoothed_da_losses = defaultdict(create_smoothed_value)
    ...
    def UpdateIterStats(self, model_out, lambda_param, inner_iter=None):
        ...
        if inner_iter is not None and self.misc_args.iter_size > 1:
            ...
            return self._UpdateIterStats_inner(model_out, inner_iter, lambda_param)
        ...
        for da_k, da_loss in model_out['DA_losses'].items():
            da_loss = da_loss.mean(dim=0, keepdim=True)
            total_da_loss += da_loss
            da_loss_data = da_loss.data[0]
            model_out['DA_losses'][da_k] = da_loss
            self.smoothed_da_losses[da_k].AddValue(da_loss_data)

        # model_out['total_loss'] = total_loss  # Add the total loss for back propagation
        model_out['total_loss'] = total_loss + lambda_param * total_da_loss  # Add the total loss for back propagation
        self.smoothed_total_loss.AddValue(total_loss.data[0])

    def _UpdateIterStats_inner(self, model_out, inner_iter, lambda_param):
        ...
        '''DA loss'''
        for da_k, da_loss in model_out['DA_losses'].items():
            assert da_loss.shape[0] == cfg.NUM_GPUS
            da_loss = da_loss.mean(dim=0, keepdim=True)
            total_da_loss += da_loss
            da_loss_data = da_loss.data[0]
            ...
            self.inner_da_losses[da_k].append(da_loss_data)
            if inner_iter == (self.misc_args.iter_size - 1):
                da_loss_data = self._mean_and_reset_inner_list('inner_DA_losses', da_k)
                self.smoothed_da_losses[da_k].AddValue(da_loss_data)
    ...
    def LogIterStats(self, cur_iter, lr, lambda_param):
        """Log the tracked statistics."""
        if (cur_iter % self.LOG_PERIOD == 0 or
                cur_iter == cfg.SOLVER.MAX_ITER - 1):
            stats = self.GetStats(cur_iter, lr, lambda_param)
    ...
    def GetStats(self, cur_iter, lr, lambda_param):
        ...
        '''DA loss'''
        head_da_losses = []
        for k, v in self.smoothed_da_losses.items():
            head_da_losses.append((k, v.GetMedianValue()))
        stats['head_DA_losses'] = OrderedDict(head_da_losses)
        stats['lambda'] = lambda_param
